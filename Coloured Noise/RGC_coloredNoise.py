...
try:
  ...
  while 1:
    ...
    if not line:
      break
    parts   = line.split(',')
    #the last element in the line always contain "\n" line break,
    #which will end up leaving the last square to always be black.
    #parts[-1]=parts[-1][0]
    ...
    if iLn == 0:
      ...
      for iB in range(1, nB+1):
        ...
        if dummie=="0":
            Frame.append((0,0,0))
        if dummie=="1":
            Frame.append((p["fIntenW"],p["fIntenW"],0))
        if dummie=="2":
            Frame.append((0,p["fIntenW"],0))
        if dummie=="3":
            Frame.append((p["fIntenW"],0,0))
