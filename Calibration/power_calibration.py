...
for iT in range(p["nTrials"]):
    for i in range (minVal,maxVal,step):   
        ...
        QDS.Scene_Render(1.0 -durMarker_s, 1, [1], [(0,0)], 0)
        print("green: " + str(i))
        #QDS.Scene_Clear(0.0, 0)
	   

    #for i in range (maxVal,minVal,-step):   
    #    p["RGB_green"] = (0,i,0)
    #    QDS.SetObjColor (1, [1], [p["RGB_green"]])
    #    QDS.Scene_Render(durMarker_s, 1, [1], [(0,0)], 1)
    #    QDS.Scene_Render(0.8 -durMarker_s, 1, [1], [(0,0)], 0)
    
    #QDS.Scene_Clear(5.0, 0)
    #for i in range (minVal,maxVal,step):   
    #    p["RGB_blue"] = (i,0,0)
    ...
    #for i in range (maxVal,minVal,-step):   
    #    p["RGB_blue"] = (i,0,0) # here red means UV led
    #    QDS.SetObjColor (1, [1], [p["RGB_blue"]])
    #    QDS.Scene_Render(durMarker_s, 1, [1], [(0,0)], 1)
    #    QDS.Scene_Render(0.8 -durMarker_s, 1, [1], [(0,0)], 0)
